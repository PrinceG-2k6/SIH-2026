    ...
    <div className="fade-in space-y-10">
      ...
      {physics.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center justify-between border-b border-[#111111] pb-2 font-mono text-xs font-bold text-[#111111] uppercase tracking-wider">
            ...
            <span className="bg-[#C41212] text-white px-2 py-0.5 text-[9px]">
          ...
          <div className="space-y-4">
            {physics.map((a) => (
              <div
                key={a.code}
                className="border-2 border-[#111111] bg-[#FAF7EE] p-5 hard-shadow relative"
              >
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#111111] pb-2">
                  <div className="flex items-center space-x-2">
                    <span className="bg-[#C41212] text-white px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider">
                      {a.severity}
                    </span>
                  ...
                  <span className="font-mono text-[10px] text-[#6B655A]">
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs bg-[#EAE2D2] border border-[#111111] p-3">
                    <div>
                      <span className="font-bold text-[#6B655A] uppercase">PRIMARY ROOT CAUSE:</span>
                      <p className="text-[#111111] mt-0.5">{a.cause}</p>
                    </div>
                    <div>
                      <span className="font-bold text-[#6B655A] uppercase">PREDICTED CONSEQUENCE:</span>
                      <p className="text-[#C41212] font-semibold mt-0.5">{a.predicted_consequence}</p>
                    </div>
                    <div className="md:col-span-2 border-t border-[#D8D0BF] pt-2">
                      <span className="font-bold text-[#6B655A] uppercase">TELEMETRY EVIDENCE:</span>
                      <p className="text-[#4D483F] mt-0.5">{a.evidence}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between border-b border-[#111111] pb-2 font-mono text-xs font-bold text-[#111111] uppercase tracking-wider">
          ...
          <span className="text-[#6B655A] text-[10px]">
            TOTAL: {alerts.alert_count} CONDITIONS
          </span>
        </div>

        {alerts.alerts.length === 0 ? (
          <div className="border-2 border-[#1b6a38] bg-[#EBF5EE] p-12 text-center">
            ...
            <p className="mt-2 font-body text-xs text-[#4D483F] max-w-md mx-auto">
          ...
          <div className="space-y-3">
            {alerts.alerts.map((a) => {
              ...
              const badgeBg = isCrit ? "bg-[#C41212]" : isWarn ? "bg-[#a65800]" : "bg-[#111111]";

              return (
                <div
                  key={`${a.code}-${a.metric}`}
                  className="border border-[#111111] bg-[#FAF7EE] p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                >
                  ...
                  <div className="font-mono text-xs text-right shrink-0 border-t sm:border-t-0 sm:border-l border-[#D8D0BF] pt-2 sm:pt-0 sm:pl-4">
                    <div className="text-[10px] text-[#6B655A] uppercase">{a.metric}</div>
                    <div className="text-sm font-bold text-[#111111]">
                      {a.value.toFixed(3)}{" "}
                      <span className="text-[10px] font-normal text-[#6B655A]">
                        (LIMIT: {a.threshold})
                      </span>
                    </div>
                  </div>
